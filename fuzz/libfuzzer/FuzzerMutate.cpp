...
MutationDispatcher::MutationDispatcher(Random &Rand,
  ...
  // binder fuzzing give me a reference to the default mutators
  if (EF->LLVMFuzzerPassDefaultMutators){
    printf("DefaultMutators %p\n", &DefaultMutators);
    EF->LLVMFuzzerPassDefaultMutators((void*)&DefaultMutators, this, (void*)&CurrentMutatorSequence); 
  }
}
...
void MutationDispatcher::PrintMutationSequence(bool Verbose) {
  ...
  for (size_t i = 0; i < EntriesToPrint; i++)
    Printf("%s-", CurrentMutatorSequence[i].c_str());
...
size_t MutationDispatcher::MutateImpl(uint8_t *Data, size_t Size,
  ...
  for (int Iter = 0; Iter < 100; Iter++) {
    ...
    if (NewSize && NewSize <= MaxSize) {
      ...
      //CurrentMutatorSequence.push_back(M.Name);
      return NewSize;
    }
  }
  *Data = ' ';
