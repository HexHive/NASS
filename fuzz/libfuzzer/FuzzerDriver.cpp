...
int FuzzerDriver(int *argc, char ***argv, UserCallback Callback) {
  ...
  if (RunIndividualFiles) {
    ...
    Printf("%s: Running %zd inputs %d time(s) each.\n", ProgName->c_str(),
           Inputs->size(), Runs);
    // HACK Replaying dumps
    const char* replay_dmp_dir = getenv("REPLAY_DUMP");
    if(replay_dmp_dir != NULL){
        DIR* dir = opendir(replay_dmp_dir);
        if(!dir){
            Printf("Error REPLAY_DUMP set but not valid path");
            exit(-1);
        }
        std::vector<std::pair<int, std::string>> numeric_files;
        struct dirent* entry; 
        while ((entry = readdir(dir)) != nullptr) {
            if (entry->d_type == DT_REG) { 
                std::string filename(entry->d_name);
                std::string::size_type dot_pos = filename.find_last_of('.');
                std::string name_without_ext = (dot_pos != std::string::npos) ? filename.substr(0, dot_pos) : filename;
                if (std::all_of(name_without_ext.begin(), name_without_ext.end(), ::isdigit)) {
                    int num = std::stoi(name_without_ext);
                    numeric_files.emplace_back(num, std::string(replay_dmp_dir) + "/" + filename);
                }
            }
        }
        ...
        std::sort(numeric_files.begin(), numeric_files.end(),
        ...
        for (const auto& file_path : sorted_paths) {
            auto StartTime = system_clock::now();
            Printf("Running: %s\n", file_path.c_str());
            RunOneTest(F, file_path.c_str(), Options.MaxLen);
            auto StopTime = system_clock::now();
            auto MS = duration_cast<milliseconds>(StopTime - StartTime).count();
            Printf("Executed %s in %ld ms\n", file_path.c_str(), (long)MS);
            usleep(5000);
        }
    }
    for (auto &Path : *Inputs) {
