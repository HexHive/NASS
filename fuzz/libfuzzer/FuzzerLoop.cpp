...
Fuzzer::Fuzzer(UserCallback CB, InputCorpus &Corpus, MutationDispatcher &MD,
  ...
  if (EF->__sanitizer_set_crash_callback)
    EF->__sanitizer_set_crash_callback(StaticCrashSignalCallback);
  if (EF->LLVMFuzzerPassStats)
    EF->LLVMFuzzerPassStats(&TotalNumberOfRuns, &NumberOfNewUnitsAdded, &LastCorpusUpdateRun);
  assert(!F);
  F = this;
  TPC.ResetMaps();
  IsMyThread = true;
...
void Fuzzer::CrashCallback() {
  ...
  PrintStackTrace();
  if(!INITED){
    Printf("IMPORTANT: crashed during replaying of seeds!\n");
  }
...
std::string Fuzzer::WriteToOutputCorpus(const Unit &U) {
  ...
  std::stringstream ss;
  ss << std::setw(10) << std::setfill('0') << TotalNumberOfRuns;
  std::string Prefix = ss.str();
  std::string Filename = Prefix + '-' + Hash(U) + '-'  + timestamp_str;
...
void Fuzzer::WriteUnitToFileWithPrefix(const Unit &U, const char *Prefix) {
  ...
  std::stringstream ss;
  ss << std::setw(10) << std::setfill('0') << TotalNumberOfRuns;
  std::string Iterations = ss.str();
  std::string Path = Options.ArtifactPrefix + Prefix + Iterations + '-' + Hash(U) + '-'  + timestamp_str;
