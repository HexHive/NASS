...
template <typename T> T ParcelWriter::generate(BaseType<T> *type) {
  ...
  if (checker.getValueFromSpecialSelfConstraint(selfConstraint, value)) {
    ...
    while (idx < count) {
      ...
      //FUCK THIS SHIT HACK CURSED CANCER
      int is_loop = 0;
      for (uint32_t i = 0; i < selfConstraint.size(); ++i) {
        Json::Value lhs = selfConstraint[i]["lhs"];
        string lhsName = lhs["name"].asString();
        int32_t lhsLoopIdx = parcelReaderWriter->getCounterLoopIdx(lhsName);
        if(lhsLoopIdx != -1){
            // this is a loop 
            FUZZER_LOGI("this is a lhs loop, making sure to not fuck up var generation!");
            int32_t value2 = (int32_t)randomUInt64(1, 0x1000);
            FUZZER_LOGD("hack generated loop coutner: %d", value2);
            is_loop = 1;
            parcelReaderWriter->storeValue(varName, value2);
            break;
        }
        Json::Value rhs = selfConstraint[i]["rhs"];
        string rhsName = rhs["name"].asString();
        int32_t rhsLoopIdx = parcelReaderWriter->getCounterLoopIdx(rhsName);
        if(rhsLoopIdx != -1){
            // this is a loop 
            FUZZER_LOGI("this is a rhs loop, making sure to not fuck up var generation!");
            int32_t value2 = (int32_t)randomUInt64(1, 0x1000);
            FUZZER_LOGD("hack generated loop coutner: %d", value2);
            is_loop = 1;
            parcelReaderWriter->storeValue(varName, value2);
