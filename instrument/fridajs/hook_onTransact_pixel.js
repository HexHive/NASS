...
function find_BBinder_transact(){
    ...
    for(let i = 0; i < processes.length; i++){
        if(processes[i].path === "/system/lib64/libbinder.so"){
            var BBinder_transact = processes[i].findExportByName("_ZN7android14IPCThreadState14executeCommandEi");
            if(BBinder_transact !== null){
                console.log("thread execute: ", BBinder_transact);
                return BBinder_transact.add(0x528); //TODO update this for new devices
            }
        }
    }
    return null;    
